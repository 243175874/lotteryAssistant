...
export default class HotAndCold extends Component {
    constructor(props) {
        super(props);
        this.emitter = null;
        this.menuList = [{ key: "冠军", value: 0 }, { key: "亚军", value: 1 }, { key: "第三球", value: 2 }, { key: "第四球", value: 3 },
        { key: "第五球", value: 4 }, { key: "第六球", value: 5 }, { key: "第七球", value: 6 }, { key: "第八球", value: 7 },
        { key: "第九球", value: 8 }, { key: "第十球", value: 9 }]
        this.state = {
            loading: false,
            list: [],//开奖历史数据
            selectedMenu: 0,
    ...
    renderMenuListView() {
        return this.menuList.map((item, index) => {
            let style = { color: this.state.selectedMenu == item.value ? "red" : "", width: "10%" };
            return (<div onClick={() => { this.setSelectedMenu(item.value) }} className="flex-center fl h100" key={index} style={style}>{item.key}</div>)
        });
    }
    ...
    render() {
        return (
            <div className="wh100" style={{ fontSize: "12px" }}>
                ...
                <div className="w100" style={{ height: "32px", background: "#EDEDED", fontWeight: "bold", fontSize: "13px", color: "#333", overflow: "auto" }}>
                    <div style={{ width: "200%" }}>
