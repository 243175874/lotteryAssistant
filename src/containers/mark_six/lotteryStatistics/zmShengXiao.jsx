...
export default class TmShengXiao extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    getLotteryRecordList(period) {
        ...
        post('/v1/api/lottery/statistics', { period }).then(data => {
            if (data.code == 200) {
                ...
                let sx_zm = this.handleRecordList(data.data);
                //热图
                let sx_zm_statistics = this.statisticsRecordList(sx_zm, shuxiang);
                this.createChart(echarts.init(document.getElementById('hot')), shuxiang, sx_zm_statistics, this.getAmountTime(sx_zm_statistics));


                //冷图
                let sx_zm_omit = this.statisticsOmitRecordList(sx_zm, shuxiang);
                this.createChart(echarts.init(document.getElementById('cold')), shuxiang, sx_zm_omit, this.getAmountTime(sx_zm_omit));
            }
    ...
    handleRecordList(data) {
        let list = [];
        data.forEach(item => {
            list.push(item.sx.slice(0, 6));//开奖正码属相
        });
        return list;
    }
    ...
    render() {
        return (
            <div className="wh100">
                ...
                <div className="w100 pr" style={{ height: "480px" }}>
