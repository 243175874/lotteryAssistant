...
export default class History extends Component {
    ...
    getHistoryService(id, number) {
        ...
        post('/v1/api/arrondi/history_dx', { id }).then(data => {
            ...
            return (
                <div key={index} className="flex" style={{ flex: "1", borderRight: "0.5px solid #E6E6E6", borderBottom: "0.5px solid #E6E6E6" }}>
                    <div className="w50 h100 flex-center">
                        <div className="flex-center" style={{ ...style, background: "#ff3344" }}>大</div>
                    </div>
                    <div className="w50 h100 flex-center">
                        <div className="flex-center" style={{ ...style, background: "#0068b7" }}>小</div>
                    </div>
                </div >
            );
        });
            ...
            return (<div className="flex" key={index} style={{ width: this.props.width, height: "30px", background: index % 2 !== 0 ? '#f5f5f9' : "white" }}>
                {
                    item.data.map((element, j) => {
                        ...
                        let big = element.v[0].key == "大" ? element.v[0].value : element.v[1].value;
                        let small = element.v[0].key == "小" ? element.v[0].value : element.v[1].value;

                        return (
                            <div className="flex" key={j} style={{ flex: "1", borderRight: "0.5px solid #E6E6E6" }}>
                                <div className="w50 h100 flex-center" style={{ color: "#ff3344" }}>{big}</div>
                                <div className="w50 h100 flex-center" style={{ color: "#0068b7" }}>{small}</div>
                            </div>)
                    })
                }
            </div>)
