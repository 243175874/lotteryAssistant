...
export default class TmBoSe extends Component {
    ...
    getLotteryRecordList(period) {
        ...
        post('/v1/api/lottery/statistics', { period }).then(data => {
            if (data.code == 200) {
                ...
                let bs_tm = this.handleRecordList(data.data);
                // 热图
                let bs_tm_statistics = this.statisticsRecordList(bs_tm, color);
                this.createChart(echarts.init(document.getElementById('hot')), color, bs_tm_statistics, this.getAmountTime(bs_tm_statistics));

                // 冷图
                let bs_tm_omit = this.statisticsOmitRecordList(bs_tm, color);
                this.createChart(echarts.init(document.getElementById('cold')), color, bs_tm_omit, this.getAmountTime(bs_tm_omit));
            }
    ...
    handleRecordList(data) {
        let list = [];
        data.forEach(item => {
            list.push(item.bs.slice(6, 7));    //开奖特码波色
        });
        return list;
    }
