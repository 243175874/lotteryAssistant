...
import { setSelectedTab, setSixTypeId, setSixUserId } from '../../../redux/action'
@connect(
    state => ({ sixMasterHandUid: state.sixUserId, sixMasterHandTid: state.sixTypeId, articleId: state.articleId, isShowDetailHead: state.isShowDetailHead }),
    { setSelectedTab, setSixTypeId, setSixUserId }
)
...
export default class SixMasterHandList extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.sixMasterHandTid == "") {
            ...
            this.props.setSixTypeId(sixMasterHandTid);
        } else {
        ...
        if (this.props.sixMasterHandUid == "") {
            ...
            this.props.setSixUserId(sixMasterHandUid);
        } else {
            localStorage.setItem("sixMasterHandUid", this.props.sixMasterHandUid);
        }
        this.getDetail(sixMasterHandTid, sixMasterHandUid);
    ...
    getDetail(cate, id) {
        this.setState({ loading: true });
        post(`/v1/api/article/info?uid=${id}&cate=${cate}`).then(data => {
            if (data.code == 200) {
                //console.log(data);
