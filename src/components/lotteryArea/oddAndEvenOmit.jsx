...
export default class OddAndEvenOmit extends Component {
    constructor(props) {
        super(props);
        this.emitter = null;
        this.state = {
    ...
    /**
       ...
       */
    statistics(list, i) {
        let oddAndEvenList = this.statisticsNumberOddAndEven(list, i);
        let oddOmitList = Common.statisticsOmitTime(oddAndEvenList, "单");
        let evenOmitList = Common.statisticsOmitTime(oddAndEvenList, "双");
        //使两个数组长度一样。
        let { omitList1, omitList2 } = Common.dealWithTwoList(oddOmitList, evenOmitList);
        this.setState({ oddOmitList: omitList1, evenOmitList: omitList2 });
    }
     ...
     * @param {第n球的索引} i 
     */
    statisticsNumberOddAndEven(list, i) {
        //console.log(list.kj_data[index]);
        let data = [];
        list.forEach(element => {
            if (Number(element.kj_data[i]) % 2 !== 0) {
                data.push("单");
            } else {
                data.push("双");
            }
        ...
        return (
            <div className="wh100" style={{ fontSize: "12px" }}>
                ...
                <div className="w100" style={{ height: "36px", fontSize: "12px", borderTop: "1px solid #E6E6E6", overflow: "auto" }}>
                    <div className="h100 flex" style={{ width: this.props.menuWidth, flexDirection: "row" }}>
                        {this.renderMenuListView()}
                    </div>
                </div>
                <div className="w100 flex" style={{ height: "45px", fontSize: "16px", color: "white" }}>
                    <div className="w50 h100 flex-center" style={{ background: "#FF3344" }}>单</div>
                    <div className="w50 h100 flex-center" style={{ background: "#0068B7" }}>双</div>
                </div>
                ...
                <div className='w100 flex clearfix'>
                    <div className="w50" style={{ height: "30px" }}>
                        {this.renderLeftListView(this.state.oddOmitList, tableBorder)}
                    </div>
                    <div className="w50" style={{ height: "30px" }}>
                        {this.renderRightListView(this.state.evenOmitList, tableBorder, finalTableCellBorder)}
                    </div>
                </div>
            </div>
        );
