...
const color = ["红", "蓝", "绿"];
export default class ZmBoSe extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
                    ...
                    label: {
                        normal: {
                            formatter: function (o) {
                                let percentage = Number(o.data.value) == 0 ? 0 : (Number(o.data.value) / dataAmount) * 100;
                    ...
                    itemStyle: {
                        ...
                        normal: {
                            color: function (params) {
                                //自定义颜色
                                var colorList = ['#F24E61', '#2D99FF', '#00CDAC'];
    ...
    getLotteryRecordList(period) {
        ...
        post('/v1/api/lottery/statistics', { period }).then(data => {
            if (data.code == 200) {
                ...
                let bs_zm = this.handleRecordList(data.data);
                //热图
                let bs_zm_statistics = this.statisticsRecordList(bs_zm, color);
                this.createChart(echarts.init(document.getElementById('hot')), color, bs_zm_statistics, this.getAmountTime(bs_zm_statistics));

                //冷图
                let bs_zm_omit = this.statisticsOmitRecordList(bs_zm, color);
                this.createChart(echarts.init(document.getElementById('cold')), color, bs_zm_omit, this.getAmountTime(bs_zm_omit));
            }
    ...
    handleRecordList(data) {
        let list = [];
        data.forEach(item => {
            list.push(item.bs.slice(0, 6));    //开奖特码波色
        });
        return list;
    }
