...
export default class ContinuousNumber extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    initRecordList(data) {
        ...
        data.forEach(item => {
            let data = [];
            data = item.sx;
            list.push({ 'year': item.year, 'period': item.period, 'data': data });
        });
        return list;
    }
    ...
    renderRecordItemView(item, index) {
        return item.data.map((element, i, array) => {

            let flag = false;

            if (i < 6) {
                let value = Math.abs(shuxiang.indexOf(array[i]) - shuxiang.indexOf(array[i + 1]));
                flag = value == 1 || value == 11 ? true : false;
                if (!flag && i > 0) {
                    let value = Math.abs(shuxiang.indexOf(array[i]) - shuxiang.indexOf(array[i - 1]));
                    flag = value == 1 || value == 11 ? true : false;
                }
            ...
            const numberStyle = {
                ...
                background: flag ? "#FF3344" : "none", color: flag ? "white" : "black"
            }

            return (
                <li key={i} className="flex-center" style={{ width: "14.2%", borderLeft: "1px solid #D6D6D6" }}>
                    <div className="flex-center" style={numberStyle}>
                        {element}
                    </div>
                </li>
            )
