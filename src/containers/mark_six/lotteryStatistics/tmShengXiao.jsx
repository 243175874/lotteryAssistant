...
export default class TmShengXiao extends Component {
    ...
    getLotteryRecordList(period) {
        ...
        post('/v1/api/lottery/statistics', { period }).then(data => {
            if (data.code == 200) {
                ...
                let sx_tm = this.handleRecordList(data.data);
                //热图
                let sx_tm_statistics = this.statisticsRecordList(sx_tm, shuxiang);
                this.createChart(echarts.init(document.getElementById('hot')), shuxiang, sx_tm_statistics, this.getAmountTime(sx_tm_statistics));

                //冷图
                let sx_tm_omit = this.statisticsOmitRecordList(sx_tm, shuxiang);
                this.createChart(echarts.init(document.getElementById('cold')), shuxiang, sx_tm_omit, this.getAmountTime(sx_tm_omit));
            }
    ...
    handleRecordList(data) {
        let list = [];
        data.forEach(item => {
            list.push(item.sx.slice(6, 7));//开奖特码属相
        });
        return list;
    }
