...
export default class zmSum extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    handleRecordList(data) {
        ...
        let weishu = [];//尾数
        data.forEach(item => {
            number.push(item.numbers.slice(0, 6));//开奖特码号码
            daxiao.push(item.dx.slice(0, 6));//开奖大小
            danshuang.push(item.ds.slice(0, 6));//开奖单双
            weishu.push(item.ws.slice(0, 6));//开奖尾数
        });
