...
export default class History extends Component {
    ...
    getHistoryService(id, number) {
        ...
        post('/v1/api/arrondi/history_lh', { id }).then(data => {
            ...
            return (
                <div key={index} className="flex" style={{ flex: "1", borderRight: "0.5px solid #E6E6E6", borderBottom: "0.5px solid #E6E6E6" }}>
                    <div className="w50 h100 flex-center">
                        <div className="flex-center" style={{ ...style, background: "#ff3344" }}>龙</div>
                    </div>
                    <div className="w50 h100 flex-center">
                        <div className="flex-center" style={{ ...style, background: "#0068b7" }}>虎</div>
                    </div>
                </div >
            );
        });
    ...
    renderListView() {
        return this.state.list.map((item, index) => {
            return (<div className="flex" key={index} style={{ width: "110%", height: "30px", background: index % 2 !== 0 ? '#f5f5f9' : "white" }}>
                {
                    item.data.map((element, j) => {
                        console.log(element.v);
                        let dragon = element.v[0].key == "龙" ? element.v[0].value : element.v[1].value;
                        let tiger = element.v[0].key == "虎" ? element.v[0].value : element.v[1].value;

                        return (
                            <div className="flex" key={j} style={{ flex: "1", borderRight: "0.5px solid #E6E6E6" }}>
                                <div className="w50 h100 flex-center" style={{ color: "#ff3344" }}>{dragon}</div>
                                <div className="w50 h100 flex-center" style={{ color: "#0068b7" }}>{tiger}</div>
                            </div>)
                    })
                }
            </div>)
            ...
            <div className="wh100" style={{ fontSize: "12px" }}>
                ...
                <div className="w100 flex" style={{ height: "60px" }}>
                    ...
                    <div className="h100" ref="title" style={{ width: "80%", overflow: "auto" }}>
                        <div className="h50 flex" style={{ width: "110%", flexDirection: "row" }}>
